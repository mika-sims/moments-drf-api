...
class PostDetailViewTest(APITestCase):
    def setUp(self):
        ...
        Post.objects.create(owner=adam, title='adams test post',)
        Post.objects.create(owner=eve, title='eves test post',)

    def test_can_retrieve_post_using_valid_id(self):
        response = self.client.get('/posts/1/')
        self.assertEqual(response.data['title'], 'adams test post')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_authenticated_user_cannot_update_other_users_post(self):
        self.client.login(username='adam', password='pass')
